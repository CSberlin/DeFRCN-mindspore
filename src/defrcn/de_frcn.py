...
from .resnet import ResNetFea, ResidualBlockUsing, build_resnet_backbone
from .gdl import AffineLayer, GradientDecoupleLayer
import cv2

class De_Frcn(nn.Cell):
    """
    defrcn Network.
    ...
    def __init__(self, config):
        ...
        self.feature_shape = config.feature_shapes
        self.roi_backward_scale = config.roi_backward_scale
        self.rpn_backward_scale = config.rpn_backward_scale
        # Anchor generator
        ...
        for anchor_base in self.anchor_base_sizes:
            anchor_generator = AnchorGenerator(anchor_base, self.anchor_scales, self.anchor_ratios)
            self.anchor_generators.append(anchor_generator)

        self.num_anchors = len(self.anchor_ratios) * len(self.anchor_scales)

        assert len(self.feature_shape) == len(self.anchor_generators)

        # Backbone
        self.backbone = build_resnet_backbone(ResidualBlockUsing, config)

        # AffineLayer
        self.rpn_gdl = GradientDecoupleLayer(self.rpn_backward_scale)
        self.roi_gdl = GradientDecoupleLayer(self.roi_backward_scale)
        self.affine_rpn = AffineLayer(num_channels=config.resnet_out_channels[2], bias=True)
        self.affine_rcnn = AffineLayer(num_channels=config.resnet_out_channels[2], bias=True)
        ...
        # res5
        self.layer4 = ResNetFea.make_layer(ResidualBlockUsing,
                                           config.resnet_block[-1],
                                           in_channel=config.resnet_in_channels[-1],
                                           out_channel=config.resnet_out_channels[-1],
                                           stride=2,
                                           training=False,
                                           weights_update=True)
        # Rcnn
        ...
        # whether froze backbone, rpn, roi_head
        if self.cfg.backbone_freeze:
            for p in self.backbone.get_parameters(expand=True):
                p.requires_grad = False
                print("froze backbone parameters")
        
        if self.cfg.rpn_freeze:
            for p in self.rpn_with_loss.get_parameters(expand=True):
                p.requires_grad = False
                print("froze rpn parameters")
        
        if self.cfg.roi_head_freeze:
            for p in self.layer4.get_parameters(expand=True):
                p.requires_grad = False
    ...
    def roi_init(self, config):
        ...
        Args:
            ...
            config.roi_align_featmap_strides (int): featmap_strides in each layer.
    ...
    def construct(self, img_data, image_id, img_metas, gt_bboxes, gt_labels, gt_valids):
        """
        construct the Defrcn Network.
        ...
        normalize_img_data = self.normalize_fn(img_data)
        x = self.backbone(normalize_img_data)
        
        features_de_rpn = self.affine_rpn(self.rpn_gdl(x))
        anchor_list = self.anchor_generators[0](self.feature_shape)

        rpn_loss, cls_score, bbox_pred, rpn_cls_loss, rpn_reg_loss, _ = self.rpn_with_loss(features_de_rpn,
                                                                                           img_metas,
                                                                                           anchor_list,
                                                                                           gt_bboxes,
        ...
        if self.training:
            proposal, proposal_mask = self.proposal_generator(cls_score, bbox_pred, anchor_list)
        else:
            proposal, proposal_mask = self.proposal_generator_test(cls_score, bbox_pred, anchor_list)
        ...
        features_de_roi = self.affine_rcnn(self.roi_gdl(x))
        if self.training:
            roi_feats = self.roi_align(rois,
                                       self.cast(features_de_roi, ms.float32),
                                       )
        else:
    ...
    def multiclass_nms(self, boxes_all, scores_all, mask_all):
        ...
        for i in range(self.test_batch_size):
            bboxes = boxes_all[i]
            scores = scores_all[i] # shape: (1000, 61)
            ...
            for j in range(self.num_classes - 1):
                # j 0->59, k 1->60 
                # _cls_scores, This step renumbered 1-60 from 0-60 to 0-59, so eval's bbox2result_1image takes 0-59
                k = j + 1
                _cls_scores = scores[::, k:k + 1:1] # （1000, 61）
    ...
    def normalize_fn(self, img):
        ...
        mean = Tensor(np.asarray([103.53, 116.28, 123.675]), ms.float32).view(3,1,1)
        std = Tensor(np.asarray([1.0, 1.0, 1.0]), ms.float32).view(3,1,1)
        return (img - mean) / std
...
class Defrcn_Infer(nn.Cell):
    def __init__(self, config):
        super(Defrcn_Infer, self).__init__()
        ...
        self.network = De_Frcn(config)
        self.network.set_train(False)

    def construct(self, img_data, img_id, img_metas=None):
        if not self.restore_bbox:
            n, _, in_h, in_w = F.shape(img_data)
            img_metas = generator_img_meta(n, self.ori_h, self.ori_w, in_h, in_w)
        output = self.network(img_data, img_id, img_metas, None, None, None)
        return output
