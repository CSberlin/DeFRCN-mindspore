...
class DenseNoTranpose(nn.Cell):
    ...
    def construct(self, x):
        if self.device_type == "Ascend":
            ...
            weight = self.cast(self.weight, ms.float32)
            bias = self.cast(self.bias, ms.float32)
...
class Rcnn(nn.Cell):
    ...
    def __init__(self,
        ...
        cls_weight = ms.common.initializer.initializer(Normal(sigma=0.01), shape=[num_classes, self.rcnn_fc_out_channels][::-1],
                                                       dtype=self.ms_type).init_data()
        reg_weight = ms.common.initializer.initializer(Normal(sigma=0.001), shape=[self.num_classes_fronted * 4,
        ...
        # self.loss_bbox = ops.SmoothL1Loss(beta=0.0)
        self.loss_bbox = ops.SmoothL1Loss(beta=1e-15)
        ...
        self.do_cls_dropout = cfg.rcnn_do_cls_dropout
        self.keep_ratio = cfg.rcnn_keep_ratio
        self.dropout = ms.ops.Dropout(self.keep_ratio)
