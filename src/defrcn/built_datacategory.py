...
METADATACATALOG : Dict[str, Dict[str, Any]] = {}
...
def get_coco_instances_meta():
    ...
    # add background for train and eval
    for k,i in thing_dataset_id_to_contiguous_id.items():
        thing_dataset_id_to_contiguous_id[k] = i + 1
    thing_dataset_id_to_contiguous_id[0] = 0
    thing_classes.insert(0, "background")
...
def get_coco_fewshot_instances_meta():
    ret = get_coco_instances_meta()
    ...
    # add background for train and eval
    for k,i in novel_dataset_id_to_contiguous_id.items():
        novel_dataset_id_to_contiguous_id[k] = i + 1
    novel_dataset_id_to_contiguous_id[0] = 0
    novel_classes.insert(0, "background")
    ...
    # add background for train and eval
    for k,i in base_dataset_id_to_contiguous_id.items():
        base_dataset_id_to_contiguous_id[k] = i + 1
    base_dataset_id_to_contiguous_id[0] = 0
    base_classes.insert(0, "background")
...
def get_builtin_metadata(dataset_name):
    if dataset_name == "coco":
        return get_coco_instances_meta()
    elif dataset_name == "coco_fewshot":
        return get_coco_fewshot_instances_meta()
    # elif dataset_name == "voc_fewshot":
    #     return get_voc_fewshot_instances_meta()
...
def register_all_coco(root="datasets"):
    ...
    for name, imgdir, annofile in METASPLITS:
        ...
        info_dict = {}
        info_dict["name"] = name
        metainfo_dict["image_root"] = info_dict["imgdir"] = os.path.join(root, imgdir)
        metainfo_dict["json_file"] = info_dict["annofile"] = os.path.join(root, annofile)
        metadata = get_builtin_metadata("coco_fewshot")
        info_dict["metadata"] = metadata
        DATACATALOG[name] = info_dict
        ...
        metainfo_dict["metadata"] = metadata
        metainfo_dict["evaluator_type"] = "coco"
        metainfo_dict["dirname"] = "datasets/coco"
        METADATACATALOG[name] = metainfo_dict